    ...
    <form method="post" class="text-center" enctype="multipart/form-data">
        ...
        ?>
            <div class="form-group row mb-3">
                <label for="namaDokter" class="col-sm-2 col-form-label offset-2">Nama Dokter</label>
                <div class="col-sm-6">
                    <input type="text" class="form-control" id="namaDokter" name="data[namaDokter]" value="<?= $data['namaDokter'] ?? '' ?>">
                </div>
            </div>
            <div class="form-group row mb-3">
                <label for="jkelaminDokter" class="col-sm-2 col-form-label offset-2">Jenis Kelamin</label>
                <?php
                $jkelamin = ['Pria', 'Wanita'];
                foreach ($jkelamin as $jk) {
                    $checked = ($data['jkelaminDokter'] ?? '') == $jk ? 'checked' : '';
                    echo ' <div class="col-sm-auto justify-content-center">
                                        <div class="form-check">
                                            <label class="form-check-label">
                                                <input type="radio" class="form-check-input mr-5" name="data[jkelaminDokter]" value="' . $jk . '" ' . $checked . '>' . $jk . '
                                            </label>
                                        </div>
                                    </div>';
                }
            ...
            <div class="form-group row mb-3">
                <label for="nohpDokter" class="col-sm-2 col-form-label offset-2">No.Hp Dokter</label>
                <div class="col-sm-6">
                    <input type="text" class="form-control" id="nohpDokter" name="data[nohpDokter]" value="<?= $data['nohpDokter'] ?? '' ?>">
                </div>
            </div>
            <div class="form-group row mb-3">
                <label for="nipDokter" class="col-sm-2 col-form-label offset-2">Foto</label>
                <div class="col-sm-6">
